...
const ProductScreen = (props) => {
  ...
  useEffect(async () => {
    ...
    if (product?.attachment.attachmentId) {
    await getImageApi(product?.attachment.attachmentId).then((r) => {
      setProductimageBase64(r);
    });
    }
  }, [dispatch, product?.attachment.attachmentId]);
                ...
                <div style={{ minWidth: '100%', height: '400px' }}>
                  <Image
                    style={{ height: '100%', width: '100%' }}
                      src={`${BACKEND_API_GATEWAY_URL}/api/v1/attachments/${product?.attachment.attachmentId}/imageView`}
            ...
            <Col md={3} style={{ borderLeft: '1px solid #eee' }}>
              <ListGroup variant='flush'>
                ...
                <ListGroupItem>
                    ...
                    <Rating value={4.5} text={`1 reviews`}></Rating>
                </ListGroupItem>
              ...
              <Card>
                <ListGroup variant='flush'>
                  ...
                  <ListGroupItem>
                    <Row>
                      <Col>Status:</Col>
                        <Col>{product.availableCount > 0 ? 'In Stock' : 'Out of Stock'}</Col>
                    </Row>
                  </ListGroupItem>
                        ...
                        <Col>
                          <Form.Control as='select' value={qty} onChange={(e) => setQty(e.target.value)}>
                            {product.availableCount > 10
                              ...
                              : [...Array(product.availableCount).keys()].map((x) => (
                                  <option key={x + 1} value={x + 1}>
                                    {x + 1}
                                  </option>
                                ))}
                  ...
                  <ListGroupItem>
                    <Button onClick={addToCartHandler} className='btn-block' type='button' disabled={product.availableCount <= 0}>
                      Add to Cart
                    </Button>
                  </ListGroupItem>
                </ListGroup>
