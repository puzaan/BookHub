...
export const getProductDetailApi = async (productId) => {
  const responseData = axios
    // .get(`${BACKEND_API_GATEWAY_URL}/api/catalog/product/${productId}`)
    .get(`${BACKEND_API_GATEWAY_URL}/api/v1/product/${productId}`)
    .then((response) => {
    return response.data;
  });
  return responseData;
...
export const getImageApi = async (imageId) => {
  ...
  const responseData = axios.get(`${BACKEND_API_GATEWAY_URL}/api/v1/attachments/${imageId}/imageView`, axiosConfig).then((response) => {
    return response.data;
  });
  return responseData;
};
...
export const getAllProductsDetailApi = async (pageNumber) => {
  const responseData = await axios.get
    // (`${BACKEND_API_GATEWAY_URL}/api/catalog/products?page=${pageNumber}&size=8`)
    (`${BACKEND_API_GATEWAY_URL}/api/v1/product/lists?page=${pageNumber}&size=8`)
    .then((response) => {
